#ifndef TAGMEMORY_TB_H
#define  TAGMEMORY_TB_H
#include "global_defines.h"

SC_MODULE (tag_memory_tb){
	sc_out <sc_uint<TAG_WIDTH>> tag_value; // We are looking for these address bits.
    sc_out <sc_uint<TAG_DEPTH>> tag_address; // Tag can be stored at this addres in any of the 4 set memories.
	sc_out <bool> ce,oe; // Assumed true polarity for ease of reading. Usuallya ctive low in practice.
    sc_out<sc_uint<ASSOCIATIVITY_WIDTH>> set_we; // Store tag at a particular set.
	sc_in_clk clk;
	void simple_write_read1();
	int cycle_count = 0;
	SC_CTOR(tag_memory_tb) {
        SC_METHOD(simple_write_read1);
		sensitive<<clk.pos();
	}
		
};
#endif
