...
//Ports
	sc_out <sc_uint<ASSOCIATIVITY>> way_match_rd, way_match_wr; // fully decoded match vector. 4'b0 is no match.
    sc_out <bool> hit;
	sc_out<bool> evict;
	...
	sc_in <sc_uint<ASSOCIATIVITY_WIDTH>> evict_way; // The address of the way to be evicted.
...
//Storage
	...
	SC_CTOR(tag_memory) 
	{
		...
		sensitive << tag_value << read_operation<<update_cycle<<lookup_cycle<<tag_address<<tag_address_wr<<way_match_wr;

		SC_METHOD(write_setmemory);
		sensitive << update_cycle << way_match_wr<<tag_address_wr<<tag_value_wr;
